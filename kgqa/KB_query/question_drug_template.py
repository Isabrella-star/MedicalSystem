...
# from regex import finditer  # 先用 regex 替换 finditer
# # 下面的 Predicate、Star、Any、Disjunction 是 refo 特有的类，用下面的“自定义适配”
# class Predicate:
#     def __init__(self, func):
#         self.func = func
#     def __call__(self, match):
#         return self.func(match.group())

# Star = lambda x: (x,)*0  # 简化实现，适配 Star 用法
# Any = lambda: regex.compile(r'.*?')  # 适配 Any 用法
# Disjunction = lambda *args: regex.compile('|'.join(args))  # 适配 Disjunction 用法
